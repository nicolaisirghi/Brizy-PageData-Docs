...
| Property         | Description                                                                                                                                                               |
| ---------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `deletable`      | Indicates whether the style can be deleted. Must be `"off"` for required or default styles.                                                                               |
| `id`             | Unique identifier for the style (e.g., `"paragraph"`).                                                                                                                    |
| `title`          | Human-readable name for the style (e.g., `"Body Text"`).                                                                                                                  |
| `fontFamily`     | Font family name. For Google Fonts, use the name in lowercase (e.g., `"poppins"`). For custom fonts, use the unique ID generated by the backend (e.g., `"fnt_839rjs01"`). |
| `fontFamilyType` | Source of the font. Common values are `"google"` for Google Fonts or `"custom"` for uploaded fonts.                                                                       |
| `fontSize`       | Font size in numeric format, used for desktop view (e.g., `16`).                                                                                                          |
| `fontSizeSuffix` | Unit for the font size. Usually `"px"` (e.g., `16px`).                                                                                                                    |
| `fontWeight`     | Font weight represented as a number between 100 and 900 (e.g., `400` for regular, `700` for bold).                                                                        |
| `lineHeight`     | Line height as a multiplier of the font size (e.g., `1.5` for 150% line height).                                                                                          |
| `letterSpacing`  | Spacing between letters in pixels or other units (e.g., `0.5`).                                                                                                           |
